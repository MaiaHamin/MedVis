...
for i in range(len(medcos)):
	...
	for story in like_p[:10]:
		like_nums[cats.find(story["Facebook Media Type"])] = like_nums[cats.find(story["Facebook Media Type"])] + 1
	for story in com_p[:10]
		com_nums[cats.find(story["Facebook Media Type"])] = com_nums[cats.find(story["Facebook Media Type"])] + 1
	for story in share_p[:10]
		share_nums[cats.find(story["Facebook Media Type"])] = share_nums[cats.find(story["Facebook Media Type"])] + 1
